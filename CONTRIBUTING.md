...
3. Run `npm ci` and `npm run install-engine` once, then `npm test`. If you changed the simulation pipeline, also run the integration tests in `tests/` against a running server.
