...
export function weaponUI({api,notice,updateCount}){
  ...
  function render(){
    ...
    $('#weapon-options').innerHTML=`<div class="weapon-grid">
      <section class="upgrade-card"><strong>Specializations</strong><p class="hint">${data.specs.length} specializations have a reference profile — SimulationCraft's own character for the season, gear, gems and enchants included. Healing specializations have none, so they cannot be ranked.${data.specs.some(s=>s.stale)?` <b>${data.specs.filter(s=>s.stale).map(s=>esc(s.label)).join(', ')}</b> still carry the previous season's gear, tens of item levels below the rest: their own ranking holds, but it is made on a weaker character.`:''}${data.specs.some(s=>s.ours)?` SimulationCraft has no profile this season for <b>${data.specs.filter(s=>s.ours).map(s=>esc(s.label)).join(', ')}</b>, so SimC Lab carries one of its own${data.specs.find(s=>s.ours)?.provenance?` built from ${esc(data.specs.find(s=>s.ours).provenance.name)}`:''}. They stand down as soon as the engine ships its own.`:''}</p><div class="upgrade-group-actions"><button class="text-button" data-all="spec">All</button><button class="text-button" data-none="spec">None</button><button class="text-button" data-only="tank">Tanks only</button></div><div class="weapon-specs">${[...byClass].map(([className,specs])=>`<div class="weapon-class"><h4>${esc(className)}</h4>${specs.map(s=>`<label class="check${s.stale?' stale':''}${s.ours?' ours':''}" ${s.stale?`title="SimulationCraft has not rebuilt this profile for the current season, so its character wears the previous season's gear."`:s.ours?`title="SimulationCraft has no profile for this specialization this season, so SimC Lab carries one of its own${s.provenance?`, built from ${s.provenance.name}`:''}."`:''}><input type="checkbox" data-group="spec" value="${esc(s.key)}" data-tank="${s.tank?'1':'0'}" checked>${esc(s.specName)}<small>${s.ours?'ours':esc(s.season)}${s.stale?' · old gear':''}${s.tank?' · tank':''}</small></label>`).join('')}</div>`).join('')}</div></section>
