...
function renderJob(job){
  ...
  let html=wow.sendButton(job)+tank.summary(job)+features.results(job)+upgrades.results(job)+crests.results(job)+weapons.results(job);
  ...
  for(let s=0;s<job.scenarios.length&&!['upgrades','crests','weapons'].includes(job.mode);s++){
  ...
  if(job.results.length&&!['upgrades','crests','weapons'].includes(job.mode))html+='<p class="result-note">± is an approximate 95% confidence interval for mean DPS. “Uncertain” means the difference is within combined statistical uncertainty, not that the builds are equal. Increase precision for small differences.</p>';
