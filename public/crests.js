...
export function crestUI({api,notice,updateCount}){
  ...
  function render(){
    ...
    $('#crest-options').innerHTML=`<div class="two-col"><label>Levels to simulate<select id="crest-levels"><option value="all" selected>Every level</option><option value="max">Fully upgraded only</option></select></label><div></div></div>
      <h4 class="upgrade-heading">Crests you have</h4><div class="crest-budget">${data.crests.currencies.map(c=>`<label><span>${icon(c)}${esc(c.name)}</span><input type="number" min="0" step="1" data-crest-have="${c.id}" placeholder="From the export"></label>`).join('')}</div>
      <p id="crest-state" class="hint">Import a character. The SimulationCraft addon's export includes your crests and each slot's upgrade discount.</p>
  ...
  function settings(){
    ...
    return {levels:$('#crest-levels').value,slots:$$('[data-crest-slot]:checked').map(e=>e.value),budget:Object.keys(budget).length?budget:undefined};
  }
  ...
  function plan(job,s,rows){
    ...
    return `<h4 class="upgrade-heading">Spend your crests in this order</h4>
      ...
      <p class="hint">${budget?'Within the crests above.':'No crest limit: every upgrade that gains, best value first.'} Change the numbers to plan again without simulating.</p>
  ...
  function results(job){
    ...
    let html=`<div class="search-summary"><strong>Crest Planner · ${job.crests.candidates.length} upgrades · ${esc(job.crests.season?.name)}</strong><p class="hint">${own} ${job.crests.levels==='max'?'Only fully upgraded items were simulated.':'Every level of each track was simulated.'}</p></div>`;
