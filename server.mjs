...
import {root,runsDir,engineStatus,prepare,Jobs,loadEnginePaths} from './lib/engine.mjs';
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(req.method==='GET'&&route==='/api/jobs')return json(res,200,[...jobs.jobs.values()].reverse().map(j=>({id:j.id,name:j.name,mode:j.mode,status:j.status,created:j.created,done:j.done,total:j.total})));
    const jobRoute=route.match(/^\/api\/jobs\/([\da-f-]{36})(\/cancel)?$/);
    if(jobRoute){const job=jobs.jobs.get(jobRoute[1]);if(!job)return json(res,404,{error:'Job not found.'});if(req.method==='POST'&&jobRoute[2])return json(res,200,await jobs.cancel(job.id));if(req.method==='GET'&&!jobRoute[2])return json(res,200,jobs.public(job));}
    ...
    const assets={'/':'index.html','/app.js':'app.js','/items.js':'items.js','/wowhead.js':'wowhead.js','/features.js':'features.js','/upgrades.js':'upgrades.js','/tank.js':'tank.js','/engine.js':'engine.js','/environment.js':'environment.js','/style.css':'style.css'};
