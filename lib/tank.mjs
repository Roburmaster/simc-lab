...
export async function calibrate(tank,simulate){
  ...
  const probe=actorTank(await simulate({auto:200000,buster:0,dot:0,heal:0,immortal:true},{iterations:20}));
  if(!probe?.health)throw new Error('The tank calibration could not read the character’s health.');
  ...
  for(let round=0;round<2;round++){
    ...
    const report=await simulate(current,{iterations:600});measured=boss(report);
    if(!(measured.sustained>0))throw new Error('The tank calibration measured no damage taken.');
