...
// Augmentation does most of its damage through its allies: Ebon Might and Prescience raise their damage, not its
// own. SimulationCraft gives a lone Augmentation simplified allies to buff, so it is ranked on the whole raid's
// damage, and a gap is read against what it adds to that raid (see the idle run in engine.mjs).
export const isSupport=info=>info?.class==='evoker'&&info?.spec==='augmentation';
...
export async function ownProfiles(talentData){
  ...
  for(const file of files.filter(f=>f.endsWith('.simc')).sort()){
    ...
    out.push({key:`${profile.info.class}-${profile.info.spec}`,file,className:tree.className,specName:tree.specName,specId:tree.specId,
      label:`${tree.specName} ${tree.className}`,class:profile.info.class,spec:profile.info.spec,tank:isTank(profile.info),support:isSupport(profile.info),
  ...
  for(const season of await seasonProfileDirs(source)){
    for(const file of (await fs.readdir(season.dir)).filter(f=>f.endsWith('.simc')&&!/_Raid\.simc$/.test(f))){
      ...
      found.set(key,{key,file,season:season.name,className:tree.className,specName:tree.specName,specId:tree.specId,label:`${tree.specName} ${tree.className}`,
        class:profile.info.class,spec:profile.info.spec,tank:isTank(profile.info),support:isSupport(profile.info),text:profile.text,gear:profile.gear,info:profile.info});
    }
  }
...
export const publicSpec=s=>({key:s.key,label:s.label,class:s.class,spec:s.spec,className:s.className,specName:s.specName,season:s.season,stale:!!s.stale,ours:!!s.ours,provenance:s.provenance||null,file:s.file,tank:s.tank,healer:!!s.healer,support:!!s.support});
...
// spec. Healers add one step between them, the run that reads their weapons' stats; a support specialization adds
// its idle run per scenario.
export const weaponSteps=(plan,scenarios)=>plan.specs.reduce((n,s)=>s.healer?n:n+scenarios*((s.candidates.length>plan.finalists?2:1)+(s.support?1:0))+(s.tank?1:0),0)+(plan.specs.some(s=>s.healer)?1:0);
...
// `scale`, when given, is what a percentage is taken of instead of the best result: a support specialization's
// share of the raid's damage rather than the whole raid's.
export function rankWeaponRows(rows,tank,group,inSet=()=>false,scale=null){
  ...
  const distance=(from,row)=>{const gap=value(from)-value(row);return tank?.boss?gap:100*gap/Math.max(1,scale||value(from));};
