...
export function weaponCandidates(spec,season,catalog,{level,kinds:wanted,craftedStats=[]}){
  const found=new Map();
  for(const {item,source} of season.entries){
    ...
    for(const slot of placements(item,spec,catalog)){
      ...
      for(const stat of stats){
        ...
        const bonuses=[...(item.bonusLists||[]),...(stat?[stat.bonusId]:[]),level.bonusId];
        ...
        found.set(line,{slot,kind,itemId:item.id,name:item.name,itemLevel:level.itemLevel,weaponType:weaponKinds[item.inventoryType],value,line,sources:[label],...(stat?{craftedStat:stat.name}:{})});
      }
    }
  }
...
export async function prepareWeapons(request,catalog,season,talentData,source,scenarios=1){
  ...
  for(const key of keys){
    ...
    const candidates=weaponCandidates(spec,season,catalog,{level:level.level,kinds:wantedKinds,craftedStats});
  ...
  return {season:season.season,level:{track:level.track.id,level:level.level.level,itemLevel:level.level.itemLevel,label:level.label},kinds:wantedKinds,craftedStats,finalists,specs,skipped,candidates:total};
}
