...
import {eligible,placements,carried,weaponKinds,trackLevel,raidDrop,sourceKinds} from './upgrades.mjs';
...
export async function loadReferenceSpecs(source,talentData){
  ...
  // A specialization SimC has not rebuilt for the current season keeps the previous season's character, tens of
  // item levels below the rest. Its own ranking still holds, but it is not the same character as the others.
  const newest=list.map(s=>s.season).sort((a,b)=>Number(b.match(/\d+$/)[0])-Number(a.match(/\d+$/)[0]))[0];
  for(const spec of list)spec.stale=spec.season!==newest;
...
export const publicSpec=s=>({key:s.key,label:s.label,class:s.class,spec:s.spec,className:s.className,specName:s.specName,season:s.season,stale:!!s.stale,file:s.file,tank:s.tank});
  ...
  for(const {item,source} of season.entries){
    ...
    for(const slot of placements(item,spec,catalog)){
      ...
      for(const stat of stats){
        ...
        found.set(key,{slot,kind,itemId:item.id,name:item.name,itemLevel:level.itemLevel,levelLabel:level.label,weaponType:weaponKinds[item.inventoryType],value,line,sources:[label],...(stat?{craftedStat:stat.name}:{})});
      }
    }
  }
...
export function dropLevels(season,options={}){
  ...
  const craftedLevel=Number(chosen.crafted?.itemLevel??top('Hero').levels.at(-1).itemLevel);
  ...
  const of={
    ...
    crafted:{of:()=>({ilevel:craftedLevel,itemLevel:craftedLevel,label:`item level ${craftedLevel}`})}
  };
  return {of,equal:false,public:{equal:false,
    ...
    crafted:{itemLevel:craftedLevel,label:`item level ${craftedLevel}`}}};
}
...
export async function prepareWeapons(request,catalog,season,talentData,source,scenarios=1){
  ...
  const drops=dropLevels(season,options);
  ...
  for(const key of keys){
    ...
    const candidates=weaponCandidates(spec,season,catalog,{drops:drops.of,kinds:wantedKinds,craftedStats});
  ...
  return {season:season.season,sources:drops.public,equal:drops.equal,levels:{min:Math.min(...specs.flatMap(s=>s.candidates.map(c=>c.itemLevel))),max:Math.max(...specs.flatMap(s=>s.candidates.map(c=>c.itemLevel)))},kinds:wantedKinds,craftedStats,finalists,specs,skipped,candidates:total};
}
...
export function rankWeaponRows(rows,tank,group){
