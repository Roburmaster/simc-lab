...
// A free node costs nothing even when a string marks it purchased: SimC's own hashes do that for some keystones.
const granted=(n,level=90)=>!!n.freeNode&&(!n.freeLevel||level>=n.freeLevel);
const spends=(n,s,level)=>!!s?.purchased&&!granted(n,level);
// Hero nodes outside the selected hero tree stay in the string but do nothing, exactly as SimC treats them
// (player.cpp: allocated hero talents are disabled unless their sub tree is active), so they are neither
// checked nor counted.
const activeHero=(tree,hero)=>tree.heroNodes.filter(n=>n.subTreeId===hero);
export function pointTotals(build,tree){
  const totals={class:0,spec:0,hero:0};
  for(const [key,nodes] of [['class',tree.classNodes],['spec',tree.specNodes],['hero',activeHero(tree,selectedHero(build,tree))]])for(const n of nodes){const s=build.selected[n.id];if(spends(n,s))totals[key]+=s.rank;}
  return totals;
}
...
export function validateBuild(build,tree,{budgets,level=90,entries,locks={},baseline}={}){
  ...
  for(const [id,s] of Object.entries(selected)){
    ...
    if(n.subTreeId&&n.subTreeId!==hero)continue;
    ...
    if(granted(n,level)){if(s.rank!==1)errors.push(`${n.name} must be granted.`);}else if(!s.purchased)errors.push(`${n.name} cannot be granted for free.`);
    if(n.requiresNode&&nodes.has(n.requiresNode)&&!selected[n.requiresNode])errors.push(`${n.name} requires another talent.`);
    if(entries&&n.type!=='subtree'&&!n.entries.every(e=>e.id==null||entries.has(e.id)))errors.push(`SimC data does not contain ${n.name}.`);
  }
  ...
  for(const [key,group] of [['class',tree.classNodes],['spec',tree.specNodes],['hero',activeHero(tree,hero)]]){
    for(const n of group)if(granted(n,level)&&!selected[n.id])errors.push(`Missing granted talent ${n.name}.`);
    const bought=new Set(group.filter(n=>selected[n.id]&&!spends(n,selected[n.id],level)).map(n=>n.id));
    const pending=group.filter(n=>spends(n,selected[n.id],level));let spent=0,progress=true;
