...
const weaponKinds={17:'two',15:'two',26:'two',13:'one',21:'one',22:'one',14:'shield',23:'held'};
...
function carried(value,item,bonuses,bonusSockets={}){
...
function trackLevel(data,choice,label){
...
export function selectFinalists(candidates,screen,size,tank){
  ...
  const chosen=[];
  for(const {c} of promising){const g=group(c.slot);const n=perGroup.get(g)||0;if(n>=Math.max(4,Math.ceil(size/8)))continue;perGroup.set(g,n+1);chosen.push(c);if(chosen.length>=size)break;}
  return chosen;
}
