...
export async function loadSeason(directory,catalog,source){
  ...
  return {season:{id:season.id,name:season.name},crests,tracks,difficulties,raids,dungeons,delves:delves?{id:delves.id,name:delves.name}:null,crafted:crafted?{id:crafted.id,name:crafted.name}:null,craftedStats:stats,entries,weaponSpecs,expansion,bonusSockets};
}
