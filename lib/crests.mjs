...
export function buildCrestCandidates(profile,text,request,season,catalog){
  ...
  for(const slot of allSlots){
    ...
    // An item level override hides the real upgrade level, so such an item cannot be upgraded here.
    if(/(?:^|,)ilevel=/.test(equipped.value))continue;
    ...
    let crests=0,full=0,currencyId=null;
    for(const to of track.levels.filter(l=>l.level>from.level)){
      ...
      if(levels==='max'&&to.level!==to.max)continue;
    ...
    items.push({slot,itemId:equipped.id,name,track:track.name,level:from.level,max:from.max,itemLevel:from.itemLevel,crests,fullCrests:full,currencyId});
  }
  ...
  const budget=normalizeBudget(options.budget,season)||state.owned;
  return {candidates:list,items,levels,state:{exported:state.exported,watermarks:!!state.watermarks},budget:budget||null,currencies:season.crests?.currencies||[]};
}

export function normalizeBudget(budget,season){
  if(budget===undefined||budget===null)return null;
  if(typeof budget!=='object')throw new Error('Crests you have: enter whole numbers.');
  ...
  for(const [id,value] of Object.entries(budget)){
    ...
    if(!Number.isInteger(n)||n<0||n>100000)throw new Error('Crests you have: enter whole numbers from 0.');
    result[id]=n;
  }
