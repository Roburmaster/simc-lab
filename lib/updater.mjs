...
const sourceFiles=['engine/dbc/generated/item_data.inc','engine/dbc/generated/permanent_enchant.inc','engine/dbc/generated/trait_data.inc','engine/dbc/generated/item_bonus.inc','SpellDataDump/build_info.txt'];
...
export function plan({installedWow,local,head,nightly,live,mode='auto'}){
  const target=installedWow||live?.wowBuild||head?.wowVersion;
  ...
  if(mode==='source'){if(!head)throw new Error('Could not reach the SimC source repository.');engine={kind:'source',...head};}
    ...
    if(nightly&&fits(nightly.build)&&(!head||nightly.sha===head.sha||!fits(head)))engine={kind:'nightly',...nightly.build,sha:nightly.sha,date:nightly.date,url:nightly.url,name:nightly.name};
    else if(head&&fits(head))engine={kind:'source',...head};
    else if(mode==='nightly'&&nightly)engine={kind:'nightly',...nightly.build,sha:nightly.sha,date:nightly.date,url:nightly.url,name:nightly.name};
  }
  const engineBuild=engine?.wowVersion||local?.wowVersion;
  const upToDate=!!local?.commit&&!!engine&&local.commit.startsWith(engine.sha)||mode==='data';
  const dataCurrent=!!live&&live.contentHash===local?.dataHash&&live.wowBuild===engineBuild;
  if(!engine&&mode!=='data')return {action:'wait',engine:null,target,reason:target?`SimC has not been updated for WoW ${target} yet. Try again later.`:'SimC could not be reached.'};
