...
export class Jobs {
  ...
  async add(plan,request){
    ...
    const steps=plan.weapons?weaponSteps(plan.weapons,plan.scenarios.length):plan.crests?plan.scenarios.length:(plan.upgrade?upgradeSteps(plan.upgrade,plan.scenarios.length):plan.variants.length*plan.scenarios.length);
    const job={id:randomUUID(),name:plan.profile.info.name,mode:request.mode || 'quick',created:new Date().toISOString(),status:'queued',done:0,total:steps+(plan.settings.tank?1:0),engine:plan.engine,settings:plan.settings,scenarios:plan.scenarios,search:plan.search,results:[],log:'',warnings:plan.profile.warnings,armory:isArmoryProfile(request.profile)};
  ...
  async stage(job,plan,s,stage,list,settings,spec){
    ...
    job.current={name:`${spec?spec.label+' · ':''}${job.crests?'Upgrades':stage===1?'Screening':stage===3?'Embellishment pairs':'Final round'} · ${list.length} candidates`,scenario,index:job.stages.length};
