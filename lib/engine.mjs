...
export class Jobs {
  ...
  async pump(){
    ...
    finally{job.finished=new Date().toISOString();delete job._plan;delete job.current;this.child=null;this.running=false;this.active=null;await this.save(job);void this.pump();}
  }
  ...
  execute(input,dir,job,timeout=3600000){return new Promise((resolve,reject)=>{
    const child=spawn(executable,[input],{cwd:dir,windowsHide:true,shell:false});this.child=child;let tail='';
    const capture=data=>{tail=(tail+data.toString()).slice(-12000);job.log=tail;};child.stdout.on('data',capture);child.stderr.on('data',capture);
