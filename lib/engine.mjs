...
export class Jobs {
  ...
  async add(plan,request){
    ...
    const steps=plan.weapons?weaponSteps(plan.weapons,plan.scenarios.length):(plan.upgrade?2*plan.scenarios.length:plan.variants.length*plan.scenarios.length);
    ...
    if(plan.weapons)Object.assign(job,{weapons:{...plan.weapons,screen:screenSettings(plan.settings),specs:plan.weapons.specs.map(s=>({...publicSpec(s),tank:!!s.tank,candidates:s.candidates.map(({line,...c})=>c)}))},stages:[]});
  ...
  async runWeapons(job,plan){
    for(const spec of plan.weapons.specs){
      ...
      for(let s=0;s<plan.scenarios.length&&job.status!=='cancelled';s++){
        ...
        if(job.status==='cancelled')return;
        this.rankWeapons(job,spec,s,settings.tank);
        await this.save(this.public(job));
      }
    }
  }
  ...
  rankWeapons(job,spec,s,tank){
    const rows=job.results.filter(r=>r.spec===spec.key&&r.scenario===s);
    const finals=new Set(rows.filter(r=>r.stage===2).map(r=>r.key));
    for(const row of rows){row.superseded=row.stage===1&&finals.has(row.key);row.screened=row.stage===1;delete row.rank;delete row.tier;delete row.behind;delete row.tied;delete row.variant;}
    ...
    for(const slot of new Set(spec.candidates.map(c=>c.slot)))rankWeaponRows(live.filter(r=>byKey.get(r.key)?.slot===slot),tank,group);
  }
  async stage(job,plan,s,stage,list,settings,spec){
    ...
    job.current={name:`${spec?spec.label+' · ':''}${stage===1?'Screening':'Final round'} · ${list.length} candidates`,scenario,index:job.stages.length};
    ...
    try{
      ...
      const result=profilesetResults(report,settings.tank);
