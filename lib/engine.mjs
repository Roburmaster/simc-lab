...
export class Jobs {
  ...
  rankWeapons(job,spec,s,tank){
    ...
    for(const row of rows){row.superseded=row.stage===1&&finals.has(row.key);row.screened=row.stage===1;delete row.rank;delete row.tier;delete row.behind;delete row.tied;}
    rankWeaponRows(rows.filter(r=>!r.superseded),tank);
  }
