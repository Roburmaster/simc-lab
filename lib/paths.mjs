...
export async function currentProfileDir(source){
  ...
  const seasons=entries.filter(e=>e.isDirectory()&&/^[A-Z]{2,}\d+$/.test(e.name)).map(e=>({name:e.name,prefix:e.name.replace(/\d+$/,''),n:Number(e.name.match(/\d+$/)[0])}));
  if(!seasons.length)throw new Error('The SimC engine has no season profiles.');
  const latest=seasons.sort((a,b)=>b.n-a.n)[0];
  return path.join(source,'profiles',latest.name);
}
