...
function specSection(job,spec,scenario){
  ...
  const chip=(row,worst,sets)=>{
    ...
    if(!c)return '';
    const value=healer?`${row.score.toFixed(2)} % score`:tanky?`${signed(row.score)} score`:`${number(row.dps)} DPS`;
    const unit=tanky?'':' %';
  ...
  return `<section class="spec" id="${escape(anchor(spec.key))}" style="--class:${escape(classColors[spec.class]||'#f3b754')}">
    <header class="spec-head">
      ...
      <p class="spec-meta">${rows.length} weapons${healer?` · stat score, ${job.weapons.healer?.weight??70} % healing · not simulated`:''}${spec.gear?` · reference gear ${spec.gear.itemLevel} ilvl`:''}${baseline?` · ${number(baseline.dps)} DPS`:''}${tanky?' · DPS and survival':''} · ${escape(spec.file.replace(/\.simc$/,''))}</p>
    </header>
