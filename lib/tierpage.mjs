...
// Wowhead takes the bonus IDs from the candidate's own SimC line, so the tooltip shows the simulated item.
function itemUrl(candidate){
  const bonus=candidate.value.match(/(?:^|,)bonus_id=([\d/]+)/)?.[1];
  return `https://www.wowhead.com/item=${Number(candidate.itemId)}${bonus?`?bonus=${bonus.replaceAll('/',':')}`:''}`;
}
...
function specSection(job,spec,scenario){
  ...
  const groups=tiers.map(t=>({tier:t.tier,rows:rows.filter(r=>r.tier===t.tier)})).filter(g=>g.rows.length);
  const chip=row=>{
    ...
    return `<li class="chip${row.rank===1?' first':''}"><a href="${escape(itemUrl(c))}" target="_blank" rel="noopener noreferrer">${escape(c.name)}</a>
  ...
  return `<section class="spec" id="${escape(anchor(spec.key))}">
    <h3>${escape(spec.specName)}<span class="spec-meta">${escape(spec.className)} · ${rows.length} weapons · ${escape(spec.file.replace(/\.simc$/,''))}${baseline?` · reference ${number(baseline.dps)} DPS`:''}${tanky?' · ranked on DPS and survival':''}</span></h3>
    ${groups.map(g=>`<div class="tier-row"><span class="tier tier-${g.tier}">${g.tier}</span><ul class="chips">${g.rows.map(chip).join('')}</ul></div>`).join('')}
  </section>`;
}
...
<footer>
Built by <a href="https://mythicpersona.com/simc-lab" target="_blank" rel="noopener noreferrer">SimC Lab</a> · powered by <a href="https://github.com/simulationcraft/simc" target="_blank" rel="noopener noreferrer">SimulationCraft</a> · item links go to Wowhead.<br>
Reference characters are SimulationCraft's own profiles, one per specialization, so the gear outside the weapon is theirs and not yours. Crafted weapons are shown at the secondary stats that served them best. Weapons are compared like for like with what the reference profile wields.<br>
