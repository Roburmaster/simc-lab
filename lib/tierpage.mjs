...
function specSection(job,spec,scenario){
  ...
  const healer=!!spec.healer,tanky=!healer&&rows.some(r=>Number.isFinite(r.score));
  ...
  // A healer's two-hander carries both hands' stats, so it is its own list beside the one-handers.
  const hands=(healer?[['two','Two-hand'],['one','One-hand'],['off','Off hand']]:[['main_hand','Main hand'],['off_hand','Off hand']])
    .map(([slot,name])=>({slot,name,rows:rows.filter(r=>(healer?byKey.get(r.key)?.hand:byKey.get(r.key)?.slot)===slot)})).filter(h=>h.rows.length);
  const chip=(row,worst,sets)=>{
    const c=byKey.get(row.key);
    if(!c)return '';
    const value=healer?`${row.score.toFixed(2)} % score`:tanky?`${signed(row.score)} score`:`${number(row.dps)} ${spec.support?'raid ':''}DPS`;
    const unit=tanky?'':' %';
    // A set weapon's lead is the set bonus. It is shown as such, and the rest are measured from the best without one.
    const behind=c.set&&row.behind<0?`+${(-row.behind).toFixed(2)}${unit} with the set`:row.behind===0?(sets?'best without a set bonus':'best in hand'):row.behind<0?'best in hand':`−${row.behind.toFixed(2)}${unit}`;
    // How much of the field a weapon keeps: full bar for the best, empty for the one furthest behind.
    const fill=Math.max(5,Math.min(100,Math.round(100*(1-row.behind/(worst||1)))));
    return `<li class="chip${row.rank===1?' first':''}${row.rank<=3?' podium':''}">
      ...
      <span class="body">
        ...
        <span class="meta">${escape(kindNames[c.kind]||c.kind)}${c.craftedStat?` · <b>${escape(c.craftedStat)}</b>`:''} · ${escape(c.sources[0]||'')}${c.sources.length>1?` <em>+${c.sources.length-1}</em>`:''}${c.itemLevel?` · <b>${c.itemLevel}</b>${c.levelLabel?` ${escape(c.levelLabel)}`:''}`:''}</span>
  ...
  return `<section class="spec" id="${escape(anchor(spec.key))}" style="--class:${escape(classColors[spec.class]||'#f3b754')}">
    <header class="spec-head">
      ...
      <p class="spec-meta">${rows.length} weapons${healer?` · stat score, ${job.weapons.healer?.weight??70} % healing · not simulated`:''}${spec.gear?` · reference gear ${spec.gear.itemLevel} ilvl`:''}${baseline?` · ${number(baseline.dps)} ${spec.support?'raid ':''}DPS`:''}${tanky?' · DPS and survival':''} · ${escape(spec.file.replace(/\.simc$/,''))}</p>
    </header>
    ${spec.support?`<p class="ours">Ranked on the whole raid's damage: ${escape(spec.specName)} does most of its damage through its allies' buffs, so it is simulated with SimulationCraft's simplified raid and the gaps are percent of what it adds to that raid${share?` (${number(share)} DPS)`:''}.</p>`:''}
    ${healer?`<p class="ours">Scored, not simulated: SimulationCraft cannot simulate healing. SimC read each weapon's stats; healing is scored on ${escape(spec.provenance?.name||'QE Live')}'s stat weights, damage on SimC Lab's own model. Equip and use effects are not in the score.${spec.compare?` ${spec.compare.lead>=0?'The best two-hander leads':'The best one-hand and off hand lead'} by ${Math.abs(spec.compare.lead).toFixed(2)} %.`:''}</p>`:''}
    ...
    ${hands.map(hand=>{
      const worst=Math.max(...hand.rows.map(r=>r.behind),0.01),sets=hand.rows.some(r=>byKey.get(r.key)?.set);
      return `${hands.length>1?`<h4 class="hand">${escape(hand.name)} <span>${hand.rows.length} weapons</span></h4>`:''}${
        tiers.map(t=>({tier:t.tier,rows:hand.rows.filter(r=>r.tier===t.tier)})).filter(g=>g.rows.length)
          .map(g=>`<div class="tier-row tier-${g.tier}"><span class="tier">${g.tier}</span><ul class="chips">${g.rows.map(row=>chip(row,worst,sets)).join('')}</ul></div>`).join('')}`;
    }).join('')}
  </section>`;
}
...
export function tierListPage(job){
  ...
  const levels=job.weapons.levels||{min:0,max:0},sources=job.weapons.sources||{};
  ...
  const title=`Weapon tier list · ${escape(job.weapons.season?.name||'')} · ${levelText}`;
...
.set{display:block;margin-top:6px;font-size:10.5px;line-height:1.45;color:#e9cf9c}
...
<header class="hero">
  ...
  <p class="lede">${sources.equal
    ? `Every candidate pinned to item level ${levels.max} (${escape(sources.label||'')}), equal footing: this answers what a weapon is worth, not what you can reach with it — some of these cannot be had at that level in the game.`
    : `Each weapon at the level its own source can give it: ${escape([sources.raid&&`raid ${sources.raid.label}`,sources.mplus&&`Mythic+ ${sources.mplus.label}`,sources.delves&&`delves ${sources.delves.label}`,sources.crafted&&`crafted ${sources.crafted.label}`].filter(Boolean).join(', '))}. A delve weapon cannot be pushed onto the Myth track, so it is never shown there.`}
    Each hand is ranked on its own, against what could take the same hand. Simulated locally with SimulationCraft ${escape(job.engine.version)} on WoW ${escape(job.engine.wowVersion)}.</p>
  <div class="stats">
    ${stat(String(specs.length),'specializations')}
    ${stat(String(ranked),'weapons ranked')}
    ${stat(levels.min===levels.max?String(levels.max):`${levels.min}–${levels.max}`,'item level')}
    ${stat(number(job.settings.iterations),'iterations')}
    ${stat(when,'simulated')}
  </div>
  <p class="legend">Distance behind the best weapon of the same hand:${tiers.map((t,i)=>`<b class="tier-${t.tier}" style="background:${['linear-gradient(160deg,#ffe2a6,#f3b754)','linear-gradient(160deg,#b6f0d2,#7fd7ad)','linear-gradient(160deg,#bcd9f7,#86b3e4)','linear-gradient(160deg,#ccd3df,#9aa6b8)','linear-gradient(160deg,#d8c0c8,#a98d98)'][i]}">${t.tier}</b>${Number.isFinite(t.behind)?`under ${t.behind}${i<tiers.length-1?',':''}`:'and beyond'}`).join(' ')} — in percent of DPS, or in score points where a tank is ranked on damage and survival. “~” marks a gap inside the combined 95% uncertainty. A weapon that completes an item set with the reference gear carries that set's bonus, and every other weapon in its hand loses it; it is marked, and the rest of the hand is measured from the best weapon without one.</p>
</header>
...
Reference characters are SimulationCraft's own profiles, one per specialization — the gear, gems and enchants its authors assembled for the season, so what surrounds the weapon is theirs and not yours.${specs.some(s=>s.stale)?` ${specs.filter(s=>s.stale).map(s=>escape(s.label)).join(', ')} are marked: their profile is a season behind.`:''} Crafted weapons are shown at the secondary stats that served them best. Weapons are compared like for like with what the reference profile wields.${specs.some(s=>s.healer)?` Healing specializations are not simulated — SimulationCraft cannot simulate healing — but ranked on a stat score: healing from ${escape(specs.find(s=>s.healer).provenance?.name||'QE Live')}'s stat weights, damage from SimC Lab's own model, ${job.weapons.healer?.weight??70} % healing.`:''}<br>
