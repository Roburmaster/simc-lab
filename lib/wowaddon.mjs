...
import {dataFile,addSim,removeSim,limits,schemaVersion} from './wowdata.mjs';
...
// The addon has its own release channel, so a fix for the game does not need a new app: tags named
// addon-v* publish one addon.json holding every file, its SHA-256 and the data schema it understands.
export const releasesUrl='https://api.github.com/repos/Roburmaster/simc-lab/releases';
export const manifestLimits={bytes:4*1024*1024,files:64,fileBytes:512*1024};
const sha256=text=>createHash('sha256').update(text,'utf8').digest('hex');

// Nothing is installed from a manifest until it is whole: the right addon, a schema this app writes, sane
// file paths inside the addon folder, and a SHA-256 for every file that matches its text.
export function validateManifest(manifest){
  if(!manifest||typeof manifest!=='object')throw new Error('The addon manifest is not readable.');
  if(manifest.addon!==addonName)throw new Error('The manifest is for another addon.');
  if(typeof manifest.version!=='string'||!/^\d+\.\d+\.\d+$/.test(manifest.version))throw new Error('The manifest has no version.');
  if(manifest.schemaVersion!==schemaVersion)throw new Error(`That addon expects data schema ${manifest.schemaVersion}, and this SimC Lab writes ${schemaVersion}. Update the app instead.`);
  if(!Array.isArray(manifest.files)||!manifest.files.length||manifest.files.length>manifestLimits.files)throw new Error('The manifest lists no usable files.');
  const seen=new Set();
  for(const file of manifest.files){
    if(!file||typeof file.path!=='string'||typeof file.text!=='string')throw new Error('A file in the manifest is incomplete.');
    const rel=file.path.replaceAll('\\','/');
    if(rel!==file.path||rel.startsWith('/')||rel.split('/').some(part=>!part||part==='.'||part==='..')||path.isAbsolute(rel))throw new Error(`The manifest holds an unusable path: ${file.path}`);
    if(!/^[\w./-]+$/.test(rel)||!/\.(lua|toc|txt|md|xml)$/i.test(rel))throw new Error(`The manifest holds an unexpected file: ${file.path}`);
    if(seen.has(rel))throw new Error(`The manifest lists ${rel} twice.`);
    seen.add(rel);
    if(Buffer.byteLength(file.text,'utf8')>manifestLimits.fileBytes)throw new Error(`${rel} is too large.`);
    if(typeof file.sha256!=='string'||sha256(file.text)!==file.sha256)throw new Error(`${rel} does not match its checksum.`);
  }
  if(!seen.has(`${addonName}.toc`))throw new Error('The manifest has no TOC.');
  const toc=manifest.files.find(f=>f.path===`${addonName}.toc`);
  if(toc.text.match(/^##\s*Version:\s*(.+?)\s*$/mi)?.[1]!==manifest.version)throw new Error('The TOC version and the manifest version differ.');
  return manifest;
}
...
export class WowAddon{
  constructor({installDir,appVersion,context,fetchImpl}){this.installDir=installDir;this.appVersion=appVersion;this.context=context;this.fetch=fetchImpl||globalThis.fetch;this.last=null;this.online=null;}

  // The newest addon release on GitHub, read at most once an hour unless asked again. Anything that goes
  // wrong is reported, never thrown at the caller: the addon inside the app always works.
  async checkOnline({force=false}={}){
    const store=await this.loadStore();
    if(!store.settings.online)return this.online={checked:Date.now(),disabled:true};
    if(!force&&this.online?.manifest&&Date.now()-this.online.checked<3600e3)return this.online;
    try{
      const list=await this.fetch(releasesUrl,{headers:{accept:'application/vnd.github+json','user-agent':'SimC Lab'},signal:AbortSignal.timeout(15000)});
      if(!list.ok)throw new Error(`GitHub answered ${list.status}.`);
      const releases=await list.json();
      const release=(Array.isArray(releases)?releases:[]).filter(r=>!r.draft&&typeof r.tag_name==='string'&&r.tag_name.startsWith('addon-v'))[0];
      if(!release)throw new Error('No addon release has been published yet.');
      const asset=(release.assets||[]).find(a=>a.name==='addon.json');
      if(!asset)throw new Error('That addon release carries no addon.json.');
      if(asset.size>manifestLimits.bytes)throw new Error('The addon manifest is too large.');
      const response=await this.fetch(asset.browser_download_url,{headers:{accept:'application/json','user-agent':'SimC Lab'},signal:AbortSignal.timeout(30000)});
      if(!response.ok)throw new Error(`The addon manifest answered ${response.status}.`);
      const text=await response.text();
      if(text.length>manifestLimits.bytes)throw new Error('The addon manifest is too large.');
      const manifest=validateManifest(JSON.parse(text));
      return this.online={checked:Date.now(),manifest,tag:release.tag_name,published:release.published_at};
    }catch(e){
      return this.online={checked:Date.now(),error:e.message};
    }
  }

  // Installs the files from a manifest, with the same rules as the shipped copy: inside the addon folder
  // only, temp file plus rename, TOC last.
  async installOnline(manifest){
    const where=await this.where();
    if(!where)throw new Error('The World of Warcraft AddOns folder was not found.');
    validateManifest(manifest);
    const files=[...manifest.files].sort((a,b)=>Number(a.path.endsWith('.toc'))-Number(b.path.endsWith('.toc')));
    await fs.mkdir(where.addon,{recursive:true});
    for(const file of files)await writeAddonFile(where,file.path.replaceAll('/',path.sep),file.text);
    await this.writeData(where);
    await this.setManage(true);
    return this.status();
  }
  ...
  async status(){
    ...
    // The newest addon can come from inside the app or from its own release on GitHub.
    const published=this.online?.manifest?.version||null;
    const newest=[shipped?.version,published].filter(Boolean).sort(compareVersions).at(-1)||null;
    return {found:!!where,addons:where?.addons||null,shipped:shipped?.version||null,installed:installed?.version||null,
      newest,fromGitHub:!!(published&&newest===published&&(!shipped||compareVersions(published,shipped.version)>0)),
      online:this.online?{checked:this.online.checked,version:published,tag:this.online.tag||null,error:this.online.error||null,disabled:!!this.online.disabled}:null,
      updateAvailable:!!(installed&&newest&&compareVersions(newest,installed.version)>0),linked,settings:store.settings,limits:{keep:limits.keep},
  ...
  // Keeps the addon in step on every start: with the button on, a missing addon is installed and an older one
  // updated, from the app's own copy or from the addon's own release, whichever is newer. With the button off,
  // or when the folder is a link elsewhere, nothing is touched.
  async autoUpdate(){
    let s=await this.status();
    if(!s.found||s.linked||!s.manage)return false;
    let installed=false;
    if(!s.installed||compareVersions(s.shipped||'0',s.installed)>0){await this.install({manage:false});installed=true;}
    if(s.settings.online){
      await this.checkOnline();
      s=await this.status();
      const published=this.online?.manifest;
      if(published&&(!s.installed||compareVersions(published.version,s.installed)>0)){
        await this.installOnline(published);
        installed=true;
      }
    }
    return installed;
  ...
  async settings(input){
    ...
    if(input.online!==undefined){next.online=!!input.online;this.online=null;}
  ...
  async captures(){
    const where=await this.where();if(!where)return {found:false,installed:null,characters:[],problems:[]};
    const installed=(await readToc(where.addon))?.version||null;
    const out=[],problems=[];
    ...
    for(const account of accounts){
      ...
      for(const [key,c] of Object.entries(db?.captures||{})){
        ...
        // An export the app cannot read is still listed, with the reason: that beats a failure on click.
        let problem=null;
        try{parseProfile(c.text);}catch(e){problem=e.message;}
        out.push({account:account.name,key,name:String(c.name||''),realm:String(c.realm||''),spec:String(c.spec||''),time:Number(c.time)||0,
          simc:String(c.simc||''),source:String(c.source||'SimulationCraft addon'),addon:c.addon?String(c.addon):null,text:c.text,checksum:checksumOk(c.text),usable:!problem,problem});
      }
      // Why a character is missing: the addon writes down what stopped its last export.
      const status=db?.captureStatus;
      if(status&&status.ok!==true&&typeof status.reason==='string')
        problems.push({account:account.name,reason:status.reason.slice(0,200),time:Number(status.time)||0,name:String(status.name||''),realm:String(status.realm||'')});
    }
    // One entry per character: the newest capture wins when several accounts hold the same one.
    const newest=new Map();
    for(const c of out.sort((a,b)=>b.time-a.time))if(!newest.has(c.key))newest.set(c.key,c);
    return {found:true,installed,characters:[...newest.values()],problems};
  }
}
...
const defaults=()=>({autoSend:false,keep:limits.defaultKeep,manage:null,online:true});
