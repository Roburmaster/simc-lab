...
async function assertInside(where,file){
  ...
  for(const part of ['',...path.dirname(rel).split(path.sep).filter(p=>p&&p!=='.')]){
    ...
    if(stat){const real=await fs.realpath(current);if(real.toLowerCase()!==path.resolve(current).toLowerCase())throw new Error(`${current} is a link to another location. SimC Lab only writes into a real SimCLab folder.`);}
  }
}
...
export class WowAddon{
  ...
  // last, so an interrupted install never looks complete to the game.
  async install(){
  ...
  // Keeps an installed addon in step with the app. Nothing is installed without the user asking once.
  async autoUpdate(){
    const s=await this.status();
    if(s.installed&&s.updateAvailable&&!s.linked){await this.install();return true;}
    return false;
  }
...
const defaults=()=>({autoSend:false,keep:limits.defaultKeep});
