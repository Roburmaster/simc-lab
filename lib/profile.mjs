...
// SimC's own reference profiles carry a few engine options an addon export never has: a starting resource,
// a pet choice or the time of day. They are accepted only for profiles read from the installed engine folder,
// never for pasted imports, and every other rule still applies.
const referenceOption = new RegExp(`^(?:timeofday|(?:${classes.join('|')})\\.[a-z_]+)$`);
// Import data, never SimC file/output/network directives. Unknown directives fail visibly.
export function parseProfile(input, { override = false, reference = false } = {}) {
  ...
  for (const [index, raw] of input.replace(/^\uFEFF/,'').split(/\r?\n/).entries()) {
    ...
    if (!fields.has(base) && !/^actions(?:\.[a-zA-Z_][\w]*)?$/.test(base) && !(reference && referenceOption.test(base))) throw new Error(`Line ${index + 1}: «${key}» is not supported in profile imports. Use simulation settings for run options.`);
  ...
  // The addon writes spec=unknown, and no talents, when the game reports no active specialization yet.
  if (!override && info.spec === 'unknown') throw new Error('The SimulationCraft addon could not read your specialization (spec=unknown), so the export has no spec or talents. In game, make sure a specialization is active, type /reload, then run /simc again.');
